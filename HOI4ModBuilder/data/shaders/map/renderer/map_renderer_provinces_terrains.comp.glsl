...
layout(std430, binding = 2) readonly buffer TerrainIdsToColorsBuffer
{
    uint terrainIdsToColors[];
};
...
void main()
{
    ...
    if (provinceId < provinceDataById.length())
        terrainId = uint(max(provinceDataById[provinceId].terrainId, 0));

    uint packedColor = 0u;
    if (terrainId < terrainIdsToColors.length())
        packedColor = terrainIdsToColors[terrainId];
