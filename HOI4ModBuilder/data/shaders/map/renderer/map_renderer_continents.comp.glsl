...
layout(std430, binding = 2) readonly buffer ContinentIdsToColorsBuffer
{
    uint continentIdsToColors[];
};
...
void main()
{
    ...
    if (provinceId < provinceDataById.length())
        continentId = uint(max(provinceDataById[provinceId].continentId, 0));

    uint packedColor = 0u;
    if (continentId < continentIdsToColors.length())
        packedColor = continentIdsToColors[continentId];
