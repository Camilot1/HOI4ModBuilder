...
#include "includes/map_renderer_strategic_region_data.comp.glsl"
...
layout(std430, binding = 2) readonly buffer StrategicRegionDataByIdBuffer
{
    StrategicRegionData strategicRegionDataById[];
};
...
void main()
{
    ...
    if (provinceId < provinceDataById.length())
        regionId = uint(max(provinceDataById[provinceId].regionId, 0));

    uint packedColor = 0u;
    if (regionId < strategicRegionDataById.length())
        packedColor = uint(strategicRegionDataById[regionId].color);
